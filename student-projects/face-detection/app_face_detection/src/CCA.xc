...
int object_localization_CCA(chanend c_dm, unsigned binImgHandle, boundBox_struct boundBox[], int area[], cog_struct cog[])
{
	int cnt=0;
	...
	{
		boundBox[i].xMin=LCD_HEIGHT;
		boundBox[i].yMin=LCD_ROW_WORDS*2;
		boundBox[i].xMax=0;
		boundBox[i].yMax=0;
		mergerTable[i]=0;
		area[i]=0;
	...
	for(i=0;i<LCD_HEIGHT;i++)
	{
		...
		c_dm :> unsigned;*/
		display_controller_image_read_line_p(c_dm,i,binImgHandle,bufferPtr);
		display_controller_wait_until_idle_p(c_dm,bufferPtr);

		for(unsigned j=0;j<LCD_ROW_WORDS*2;j++)
		{
			...
			if(j<(LCD_ROW_WORDS*2-1))
			{
				A=B;
				B=C;
				if(j<(LCD_ROW_WORDS*2-2)) C=mergerTable[rowBuff[j+2]];
				if(j==LCD_ROW_WORDS*2-2) C=0;
				if(j) rowBuff[j-1]=D;
				D=rowBuff[j];
			}
