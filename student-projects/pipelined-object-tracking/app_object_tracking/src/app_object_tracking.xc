...
void app(chanend c_dispMan[])
{
	...
	for (int i=0; i<IMAGE_COUNT+(N_STAGES-1)-1; i++){
		...
		par{
			if (i<IMAGE_COUNT-1)
			    image_processing_frame_substraction_binarisation(c_dispMan[0], image[i], imgHeight[i], image[i+1], binImage[i+1], imgHeight[i+1],THRESHOLD);
			if ((i-1)>=0 && (i-1)<IMAGE_COUNT-1)
				nCC = image_processing_CCA(c_dispMan[1], binImage[i], imgHeight[i], imgWidth[i], boundBox, area, cog);

			if ((i-2)>=0 && (i-2)<IMAGE_COUNT-1)
				annotate_image(c_dispMan[2], image[i-1], imgHeight[i-1], imgWidth[i-1], nCC_old, boundBox_old, area_old);
		}
